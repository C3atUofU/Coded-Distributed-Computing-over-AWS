...
plac_group = [frozenset(s) for s in itertools.combinations(worker_set,r)]
shuf_group = [frozenset(s) for s in itertools.combinations(worker_set,r+1)]

num_plac_groups = len(plac_group)  # Number of Shuffle/Placement Groups
...
for grp in shuf_group:
    ...
    comm_count += 1
    if rank == 0:
        if np.remainder(comm_count,500)==0:
            print comm_count
...
for grp in shuf_group:
    ...
    if rank in grp:
        for recv_node in grp:
            if rank != recv_node:
                req_files[grp][recv_node] = frozenset(grp - {recv_node})
    ...
    for grp in grps_to_encode_ordered[file]:
        ...
        for recv_node in grp:
            if rank != recv_node:
                ...
                indexing = list(np.round(np.linspace(0, msg_size, r+1)))
